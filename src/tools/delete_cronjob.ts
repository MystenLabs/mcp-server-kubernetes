...
export async function DeleteCronJob(
  ...
  try {
    const response = await k8sManager
      .getBatchApi()
      .deleteNamespacedCronJob(input.name, input.namespace);
    if (
      response.response?.statusCode !== undefined &&
      (response.response.statusCode === 200 || response.response.statusCode === 202)
    ) {
      return {
        content: [
          {
            success: true,
            message:
              `Deleted cronjob ${input.name} in namespace ${input.namespace}.` +
        ...
        content: [
          {
            success: false,
            message:
              `Failed to delete cronjob ${input.name} in namespace ${input.namespace}.` +
              (response.body?.details ? ` Details: ${response.body.details}` : ""),
          },
        ],
      };
