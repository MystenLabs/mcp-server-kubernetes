...
export async function scaleDeployment(
  ...
  try {
    const scale = k8sManager
      .getAppsApi()
      .readNamespacedDeploymentScale(input.name, input.namespace);
    (await scale).body.spec!.replicas = input.replicas;
    const result = await k8sManager
      .getAppsApi()
      .replaceNamespacedDeploymentScale(input.name, input.namespace, (await scale).body);
    if (
